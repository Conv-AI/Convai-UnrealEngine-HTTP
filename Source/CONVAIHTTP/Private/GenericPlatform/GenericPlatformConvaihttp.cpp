...
/**
 * A generic convaihttp request
 */
class FGenericPlatformConvaihttpRequest : public FConvaihttpRequestImpl
{
public:
	// IConvaihttpBase
	...
	virtual TArray64<FString> GetAllHeaders() const override { return TArray64<FString>(); }
	virtual FString GetContentType() const override { return TEXT(""); }
	virtual int64 GetContentLength() const override { return 0; }
	virtual const TArray64<uint8>& GetContent() const override { static TArray64<uint8> Temp; return Temp; }
	...
	virtual EConvaihttpRequestStatus::Type GetStatus() const override { return EConvaihttpRequestStatus::NotStarted; }
	virtual const FConvaihttpResponsePtr GetResponse() const override { return nullptr; }
	virtual void Tick(float DeltaSeconds) override {}
	virtual float GetElapsedTime() const override { return 0.0f; }
};

void FGenericPlatformConvaihttp::Init()
{
}

void FGenericPlatformConvaihttp::Shutdown()
{
}

IConvaihttpRequest* FGenericPlatformConvaihttp::ConstructRequest()
{
	return new FGenericPlatformConvaihttpRequest();
}

bool FGenericPlatformConvaihttp::UsesThreadedConvaihttp()
{
	// Many platforms use libcurl.  Our libcurl implementation uses threading.
	// Platforms that don't use libcurl but have threading will need to override UsesThreadedConvaihttp to return true for their platform.
	return WITH_CURL;
}
...
FString FGenericPlatformConvaihttp::UrlEncode(const FStringView UnencodedString)
{
...
FString FGenericPlatformConvaihttp::HtmlEncode(const FStringView UnencodedString)
{
...
FString FGenericPlatformConvaihttp::GetUrlDomainAndPort(const FStringView Url)
{
	FStringView Protocol;
	FStringView DomainAndPort;
	// split the convaihttp protocol portion from domain
...
FString FGenericPlatformConvaihttp::GetUrlDomain(const FStringView Url)
{
	FStringView Protocol;
	FStringView Domain;
	// split the convaihttp protocol portion from domain
...
FString FGenericPlatformConvaihttp::GetMimeType(const FString& FilePath)
{
...
FString FGenericPlatformConvaihttp::GetDefaultUserAgent()
{
...
FString FGenericPlatformConvaihttp::EscapeUserAgentString(const FString& UnescapedString)
{
...
TOptional<FString> FGenericPlatformConvaihttp::GetOperatingSystemProxyAddress()
{
	return TOptional<FString>();
}

bool FGenericPlatformConvaihttp::IsOperatingSystemProxyInformationSupported()
{
	return false;
}

TOptional<FString> FGenericPlatformConvaihttp::GetUrlParameter(const FStringView Url, const FStringView ParameterName)
{
	...
	if (Split(Url, TEXT('?'), Endpoint, ParamsAndFragment))
	{
		...
		do
		{
			...
			if (ParamKey == ParameterName)
			{
				...
				{
					ParameterValue = FGenericPlatformConvaihttp::UrlDecode(ParamValue);
				}
				else
				{
...
TOptional<uint16> FGenericPlatformConvaihttp::GetUrlPort(const FStringView Url)
{
...
FString FGenericPlatformConvaihttp::GetUrlPath(const FStringView Url, const bool bIncludeQueryString, const bool bIncludeFragment)
{
...
TOptional<bool> FGenericPlatformConvaihttp::IsSecureProtocol(const FStringView Url)
{
	...
	if (Split(Url, TEXT("://"), Scheme, UrlWithoutScheme))
	{
		if (Scheme == TEXT("convaihttps") || Scheme == TEXT("wss"))
		{
			bIsSecure = true;
		}
		else if (Scheme == TEXT("convaihttp") || Scheme == TEXT("ws"))
		{
			bIsSecure = false;
		}
	}
