...
public:
	...
	EConvaihttpResponseCodes::Type GetResponseCode() const { return ResponseCode; }
	/**
	 ...
	 */
	TArray64<uint8>& GetLastChunk() { return GameThreadChunk; }
...
protected:
	...
	/**
	 * Read our CONVAIHTTP response headers and store the results
	 *
	 * @return True if the action completed successfully, false otherwise
	 */
	...
	virtual bool FinishRequest(const EConvaihttpRequestStatus::Type FinalState);
	...
	/** Handles writing the convaihttp response body to our response body */
	void HandleDataAvailable(const uint64 NumBytesAvailable);
	/** Handles querying the next chunk of our convaihttp response body */
...
private:
	/**
	 * Handle receiving CONVAIHTTP status callbacks from WinHttp
	 *
	 ...
	 */
	void HandleConvaihttpStatusCallback(HINTERNET ResourceHandle, EWinHttpCallbackStatus Status, void* StatusInformation, uint32 StatusInformationLength);
	
	/** Mark our callback as a friend so they can call the above status callback function */
	friend void CALLBACK UE_WinHttpStatusConvaihttpCallback(HINTERNET, DWORD_PTR, DWORD, LPVOID, DWORD);
...
private:
	...
	TArray64<uint8> PayloadBuffer;

	/// Members for request response

	/** The response code received from our request */
	EConvaihttpResponseCodes::Type ResponseCode = EConvaihttpResponseCodes::Unknown;
