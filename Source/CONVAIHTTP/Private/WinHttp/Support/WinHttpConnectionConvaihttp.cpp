...
void FWinHttpConnectionConvaihttp::PumpMessages()
{
	...
	{
		TArray64<FString> LocalHeadersToReport = MoveTemp(HeaderKeysToReportReceived);
	...
	for (int32 NumStateChanges = 0; NumStateChanges < MaxStateChanges; ++NumStateChanges)
	{
		switch (CurrentAction)
		{
			...
			case EState::SendRequest:
			{
				if (!SendRequest())
				{
					FinishRequest(EConvaihttpRequestStatus::Failed_ConnectionError);
					return;
				}
				continue;
			}
	...
	: RequestUrl(InUrl)
{
	...
	const LPWSTR ConvaihttpVersion = NULL; // Setting to null defaults
	...
	RequestHandle = WinHttpOpenRequest(ConnectionHandle.Get(), TCHAR_TO_WCHAR(*InVerb), TCHAR_TO_WCHAR(*InPathAndQuery), ConvaihttpVersion, RequestReferrer, AcceptedMediaTypes, Flags);
...
bool FWinHttpConnectionConvaihttp::SetHeaders(const TMap<FString, FString>& Headers)
{
	...
	for (const TPair<FString, FString>& HeaderPair : Headers)
	{
		...
		if (!WinHttpAddRequestHeaders(RequestHandle.Get(), TCHAR_TO_WCHAR(*HeaderBuffer), HeadersLength, Flags))
		{
			...
			// according to the latest CONVAIHTTP1.1 RFC (7230) a blank header is not allowed.
...
bool FWinHttpConnectionConvaihttp::ProcessResponseHeaders()
{
	...
	// Read our CONVAIHTTP headers now that we have the size of them as a string
	...
	// Read our CONVAIHTTP status code now that we're done reading string headers
	{
		DWORD StatusCode = 0;
	...
	// Read our CONVAIHTTP content length now that we have our status code (if there is one)
	{
