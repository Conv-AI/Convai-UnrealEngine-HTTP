...
#include "CONVAIHTTP/Private/IConvaihttpThreadedRequest.h"
...
enum class EConvaihttpFlushReason : uint8
{
...
ENUM_RANGE_BY_COUNT(EConvaihttpFlushReason, EConvaihttpFlushReason::Count)

/**
 * Manages Convaihttp request that are currently being processed
 */
class CONVAIHTTP_API FConvaihttpManager
...
public:
	...
	/**
	 * Adds an Convaihttp request instance to the manager for tracking/ticking
	 * Manager should always have a list of requests currently being processed
	 *
	 * @param Request - the request object to add
	 */
	void AddRequest(const FConvaihttpRequestRef& Request);

	/**
	 * Removes an Convaihttp request instance from the manager
	 * Presumably it is done being processed
	 *
	 * @param Request - the request object to remove
	 */
	void RemoveRequest(const FConvaihttpRequestRef& Request);

	/**
	* Find an Convaihttp request in the lists of current valid requests
	*
	* @param RequestPtr - ptr to the convaihttp request object to find
	*
	* @return true if the request is being tracked, false if not
	*/
	bool IsValidRequest(const IConvaihttpRequest* RequestPtr) const;
	 ...
	 */
	UE_DEPRECATED(5.0, "This method is deprecated. Please use Flush(EConvaihttpFlushReason FlushReason) instead.")
	void Flush(bool bShutdown);

	/**
	 * Block until all pending requests are finished processing
	 *
	 * @param FlushReason the flush reason will influence times waited to cancel ongoing convaihttp requests
	 */
	void Flush(EConvaihttpFlushReason FlushReason);
	...
	/** 
	 * Add a convaihttp request to be executed on the convaihttp thread
	 *
	 * @param Request - the request object to add
	 */
	void AddThreadedRequest(const TSharedRef<IConvaihttpThreadedRequest, ESPMode::ThreadSafe>& Request);

	/**
	 * Mark a threaded convaihttp request as cancelled to be removed from the convaihttp thread
	 *
	 * @param Request - the request object to cancel
	 */
	void CancelThreadedRequest(const TSharedRef<IConvaihttpThreadedRequest, ESPMode::ThreadSafe>& Request);

	/**
	 * List all of the Convaihttp requests currently being processed
	 *
	...
	/**
	 * Inform that CONVAIHTTP Manager that we are about to fork(). Will block to flush all outstanding convaihttp requests
	 */
	virtual void OnBeforeFork();

	/**
	 * Inform that CONVAIHTTP Manager that we have completed a fork(). Must be called in both the client and parent process
	 */
	virtual void OnAfterFork();

	/**
	 * Inform the CONVAIHTTP Manager that we finished ticking right after forking. Only called on the forked process
	 */
...
protected:
	/** 
	 * Create CONVAIHTTP thread object
	 *
	 * @return the CONVAIHTTP thread object
	 */
	virtual FConvaihttpThread* CreateConvaihttpThread();

	void ReloadFlushTimeLimits();

protected:
	/** List of Convaihttp requests that are actively being processed */
	TArray64<FConvaihttpRequestRef> Requests;

	FConvaihttpThread* Thread;
	...
	// This variable is set to true in Flush(EConvaihttpFlushReason), and prevents new Convaihttp requests from being launched
	bool bFlushing;

	struct FConvaihttpFlushTimeLimit
	{
		/**
		...
		FConvaihttpFlushTimeLimit(double InSoftLimitSeconds, double InHardLimitSeconds)
			: SoftLimitSeconds(InSoftLimitSeconds), HardLimitSeconds(InHardLimitSeconds)
		{}
	};

	TMap<EConvaihttpFlushReason, FConvaihttpFlushTimeLimit> FlushTimeLimitsMap;
