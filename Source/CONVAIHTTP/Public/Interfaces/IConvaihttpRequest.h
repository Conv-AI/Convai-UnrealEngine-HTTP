...
#include "Interfaces/IConvaihttpBase.h"

class IConvaihttpRequest;
class IConvaihttpResponse;

namespace EConvaihttpRequestStatus
{
	/**
	 * Enumerates the current state of an Convaihttp request
	 */
	enum Type
	{
	...
	inline const TCHAR* ToString(EConvaihttpRequestStatus::Type EnumVal)
	{
		switch (EnumVal)
		{
			case NotStarted:
	...
	inline bool IsFinished(const EConvaihttpRequestStatus::Type Value)
	{
...
typedef TSharedPtr<IConvaihttpRequest, ESPMode::ThreadSafe> FConvaihttpRequestPtr;
typedef TSharedPtr<IConvaihttpResponse, ESPMode::ThreadSafe> FConvaihttpResponsePtr;

typedef TSharedRef<IConvaihttpRequest, ESPMode::ThreadSafe> FConvaihttpRequestRef;
typedef TSharedRef<IConvaihttpResponse, ESPMode::ThreadSafe> FConvaihttpResponseRef;

/**
 * Delegate called when an Convaihttp request completes
 *
 * @param Request original Convaihttp request that started things
 ...
 */
DECLARE_DELEGATE_ThreeParams(FConvaihttpRequestCompleteDelegate, FConvaihttpRequestPtr /*Request*/, FConvaihttpResponsePtr /*Response*/, bool /*bConnectedSuccessfully*/);

/**
 * Delegate called when an Convaihttp request receives a header
 *
 * @param Request original Convaihttp request that started things
 * @param HeaderName the name of the header
 * @param NewHeaderValue the value of the header
 */
DECLARE_DELEGATE_ThreeParams(FConvaihttpRequestHeaderReceivedDelegate, FConvaihttpRequestPtr /*Request*/, const FString& /*HeaderName*/, const FString& /*NewHeaderValue*/);

/**
 * Delegate called per tick to update an Convaihttp request upload or download size progress
 *
 * @param Request original Convaihttp request that started things
...
public:
	 ...
	 * Eg. (convaihttp://my.domain.com/something.ext?key=value&key2=value).
	 ...
	 */
	virtual void SetContent(const TArray64<uint8>& ContentPayload) = 0;
	 ...
	 * @param ContentPayload - payload to set.
	 */
	virtual void SetContent(TArray64<uint8>&& ContentPayload) = 0;
	...
	*	comma is inserted between old value and new value, per CONVAIHTTP specifications
	*/
	virtual void AppendToHeader(const FString& HeaderName, const FString& AdditionalHeaderValue) = 0;

	/**
	 * Sets an optional timeout in seconds for this entire CONVAIHTTP request to complete.
	 * If set, this value overrides the default CONVAIHTTP timeout set via FConvaihttpModule::SetTimeout().
	 *
	 * @param InTimeoutSecs - Timeout for this CONVAIHTTP request instance, in seconds
	 */
	virtual void SetTimeout(float InTimeoutSecs) = 0;

	/**
	 * Clears the optional timeout in seconds for this CONVAIHTTP request, causing the default value
	 * from FConvaihttpModule::GetTimeout() to be used.
	 */
	virtual void ClearTimeout() = 0;

	/**
	 * Gets the optional timeout in seconds for this entire CONVAIHTTP request to complete.
	 * If valid, this value overrides the default CONVAIHTTP timeout set via FConvaihttpModule::SetTimeout().
	 *
	 * @return the timeout for this CONVAIHTTP request instance, in seconds
	 */
	...
	/**
	 * Delegate called when the request is complete. See FConvaihttpRequestCompleteDelegate
	 */
	virtual FConvaihttpRequestCompleteDelegate& OnProcessRequestComplete() = 0;

	/**
	 * Delegate called to update the request/response progress. See FConvaihttpRequestProgressDelegate
	 */
	virtual FConvaihttpRequestProgressDelegate& OnRequestProgress() = 0;
	
	/**
	* Delegate called when the request will be retried
	*/
	virtual FConvaihttpRequestWillRetryDelegate& OnRequestWillRetry() = 0;

	/** 
	 * Delegate called to signal the receipt of a header.  See FConvaihttpRequestHeaderReceivedDelegate
	 */
	virtual FConvaihttpRequestHeaderReceivedDelegate& OnHeaderReceived() = 0;
	 ...
	 */
	virtual EConvaihttpRequestStatus::Type GetStatus() const = 0;
	 ...
	 * @return the response
	 */
	virtual const FConvaihttpResponsePtr GetResponse() const = 0;
