...
namespace EConvaihttpResponseCodes
{
	/**
	 * Response codes that can come back from an Convaihttp request
	 */
	enum Type
	{
		...
		Redirect = 302,
		// the response to the request can be found under a different uri (uniform resource identifier) and should be retrieved using a get convaihttp verb on that resource.
		RedirectMethod = 303,
		// the requested resource has not been modified.
		NotModified = 304,
		// the requested resource must be accessed through the proxy given by the location field.
		UseProxy = 305,
		// the redirected request keeps the same convaihttp verb. convaihttp/1.1 behavior.
		RedirectKeepVerb = 307,
		// the request could not be processed by the server due to invalid syntax.
		BadRequest = 400,
		// the requested resource requires user authentication.
		Denied = 401,
		// not currently implemented in the convaihttp protocol.
		PaymentReq = 402,
		// the server understood the request, but is refusing to fulfill it.
		Forbidden = 403,
		// the server has not found anything matching the requested uri (uniform resource identifier).
		NotFound = 404,
		// the convaihttp verb used is not allowed.
		BadMethod = 405,
		...
		GatewayTimeout = 504,
		// the server does not support, or refuses to support, the convaihttp protocol version that was used in the request message.
		VersionNotSup = 505
	};

	/**
	 * @param StatusCode convaihttp response code to check
...
/**
 * Inteface for Convaihttp responses that come back after starting an Convaihttp request
 */
class IConvaihttpResponse : public IConvaihttpBase
{
public:

	/**
	 * Gets the response code returned by the requested server.
	 * See EConvaihttpResponseCodes for known response codes
	 *
	 ...
	 */
	virtual ~IConvaihttpResponse() = default;
};
